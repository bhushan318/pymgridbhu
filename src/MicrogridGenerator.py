...
import numpy as np
import pandas as pd
from  pymgridbhu import Microgrid
...
class MicrogridGenerator:
    ...
    def __init__(
        ...
        self.path = path
        self.size_load = size_load
        self.cost_loss_load = cost_loss_load
        self.cost_overgeneration = cost_overgeneration
    ...
    def _get_random_file(self, path):
        ...
        file = pd.read_csv(
            path + onlyfiles[np.random.randint(low=0, high=len(onlyfiles))]
        )
    ...
    def _resize_timeseries(self, timeserie, current_time_step, new_time_step):
        ...
        index = pd.date_range(
            "1/1/2015 00:00:00",
            freq=str(int(current_time_step * 60)) + "Min",
            periods=(len(timeserie)),
        )  # , freq='0.9S')

        unsampled = pd.Series(timeserie, index=index)
        resampled = (
            unsampled.resample(rule=str(int(new_time_step * 60)) + "Min")
            .mean()
    ...
    def _generate_weak_grid_profile(
        self, outage_per_day, duration_of_outage, nb_time_step_per_year
    ):
        ...
        weak_grid_timeseries = np.random.random(
            int(nb_time_step_per_year + 1)
        )  # for a number of time steps, value between 0 and 1

        weak_grid_timeseries = [
            0 if weak_grid_timeseries[i] < outage_per_day / 24 else 1
        ...
        return pd.DataFrame(
            weak_grid_timeseries, columns=["grid_status"]
        )  # [0 if weak_grid_timeseries[i] < h_outage_per_day/24 else 1 for i in range(len(weak_grid_timeseries))]
    ...
    def _create_microgrid(self):
        ...
        if self.size_load is None:
            ...
            size_load = self.size_load

        load = self._scale_ts(
            self._get_load_ts(), size_load, scaling_method="max"
        )  # obtain dataframe of loads
        size = self._size_mg(
            load, size_load
        )  # obtain a dictionary of mg sizing components
        column_actions = []
        ...
        df_parameters["cost_loss_load"] = self.cost_loss_load
        df_parameters["cost_overgeneration"] = self.cost_overgeneration
        df_parameters["cost_co2"] = self.cost_co2
        # df_cost['cost'] = [0.0]
        df_status["load"] = [
            np.around(load.iloc[0, 0], 1)
        ]  # --> il y a doublon pour l'instant avec l'architecture PV, -> non si pas de pv la net load est juste la load
        df_status["hour"] = [0]
        ...
        if architecture["PV"] == 1:
            ...
            pv = pd.DataFrame(
                self._scale_ts(self._get_pv_ts(), size["pv"], scaling_method="max")
            )
        ...
        if architecture["battery"] == 1:
            ...
            battery = self._get_battery(
                capa=size["battery"]
            )  # return a dictionary of battery characteristic
            ...
            capa_to_charge = max(
                (
                    df_parameters["battery_soc_max"].values[0]
                    * df_parameters["battery_capacity"].values[0]
                    - df_parameters["battery_soc_0"].iloc[-1]
                    * df_parameters["battery_capacity"].values[0]
                )
                / df_parameters["battery_efficiency"].values[0],
                0,
            )
        ...
        if architecture["grid"] == 1:
            rand_weak_grid = self.week_grid
            price_scenario = np.random.randint(low=1, high=3)
            if rand_weak_grid == 1:
                architecture["genset"] = 1
            grid = self._get_grid(
                rated_power=size["grid"],
                weak_grid=rand_weak_grid,
        ...
        df_actions = {
            i: [] for i in column_actions
        }  # pd.DataFrame(columns = column_actions, )
        df_actual_production = {
            i: [] for i in column_actual_production
        }  # pd.DataFrame(columns=column_actual_production)
            ...
            if self.microgrids != []:
                ...
                for i in range(self.nb_microgrids):
                    parameters = parameters.append(
                        self.microgrids[i].parameters, ignore_index=True
                    )
        ...
        for i in range(self.nb_microgrids):
            ...
            if self.microgrids[i]._has_train_test_split == False:
                ...
                if self.microgrids[i]._has_run_mpc_baseline == True:
                    cost_mpc = np.around(
                        self.microgrids[i]._baseline_linprog_cost.sum().values[0], 1
                    )

                if self.microgrids[i]._has_run_rule_based_baseline == True:
                    cost_rule_based = np.around(
                        self.microgrids[i]._baseline_priority_list_cost.sum().values[0],
                        1,
                    )
                ...
                if self.microgrids[i]._has_run_mpc_baseline == True:
                    cost_mpc = np.around(
                        self.microgrids[i]
                        ._baseline_linprog_cost.iloc[self.microgrids[i]._limit_index :]
                        .sum()
                        .values[0],
                        1,
                    )
                ...
                if self.microgrids[i]._has_run_rule_based_baseline == True:
                    cost_rule_based = np.around(
                        self.microgrids[i]
                        ._baseline_priority_list_cost.iloc[
                            self.microgrids[i]._limit_index :
                        ]
                        .sum()
                        .values[0],
                        1,
            ...
            df_cost = df_cost.append(
                {
                    ...
                    "Cost": cost_run,
                    "Cost (MPC)": cost_mpc,
                    "Cost (rule-based)": cost_rule_based,
                },
                ignore_index=True,
            )
