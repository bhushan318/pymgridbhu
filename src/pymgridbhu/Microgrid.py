...
from pymgridbhu.algos.Control import Benchmarks
...
# commented for now, issues with parallel processes
...
class Battery:
    ...
    You can then add a point and use tab to have suggestion of the different parameters
...
class Microgrid:
    ...
    parameters : dataframe
        In parameters we find:
            ...
            -'architecture': a dictionary containing a binary variable for each possible generator and indicating if
            ...
            -'control_dict': an example of the control dictionary that needs to be passed in run to operate the
             microgrid
    ...
    ----------
        ...
        _df_record_control_dict: dataframe
            This dataframe is used to record the control actions taken at each time step.
    ...
    def _param_check(self, parameters):
        ...
        if not isinstance(parameters, dict):
            raise TypeError(
                "parameters must be of type dict, is ({})".format(type(parameters))
            )
        ...
        if not isinstance(architecture, dict):
            raise TypeError(
                "parameters['architecture'] must be of type dict, is ({})".format(
                    type(architecture)
                )
            )
        ...
        for key, val in architecture.items():
            if isinstance(val, bool):
                ...
                raise TypeError(
                    "Value ({}) of key ({}) in architecture is of unrecognizable type, "
        ...
        for key in keys:
            try:
                ...
                print(
                    "Dict of parameters does not appear to contain {} key".format(key)
                )
                raise
            if not isinstance(df, pd.DataFrame):
                raise TypeError(
                    "parameters['{}'] must be of type pd.DataFrame, is ({})".format(
                        key, type(df)
                    )
                )
    ...
    def forecast_all(self):
        ...
        if self.architecture["grid"] == 1:
            ...
            (
                forecast["grid_import"],
                forecast["grid_export"],
            ) = self.forecast_grid_prices()
    ...
    def forecast_pv(self):
        ...
        if self._data_set_to_use == "training":
            forecast = self._pv_train.iloc[
                self._tracking_timestep : self._tracking_timestep + self.horizon
            ].values.flatten()
    ...
    def run(self, control_dict):
        ...
        ----------
        control_dict : dictionary
            dictionary containing the different control actions we want to apply to the microgrid. Its fields depend
            on the architecture of the microgrid

        Return
        ----------
        self.get_updated_values(): dictionary
        ...
        self._df_record_control_dict = self._record_action(
            control_dict, self._df_record_control_dict
        )

        self._df_record_actual_production = self._record_production(
            control_dict, self._df_record_actual_production, self._df_record_state
        )

        if self.architecture["grid"] == 1:
            self._df_record_co2 = self._record_co2(
                {
                    i: self._df_record_actual_production[i][-1]
                    for i in self._df_record_actual_production
                },
                self._df_record_co2,
                self.grid.co2,
            )
            ...
            self._df_record_cost = self._record_cost(
                ...
                self._df_record_co2,
                self.grid.price_import,
                self.grid.price_export,
            )
            self._df_record_state = self._update_status(
                control_dict,
                self._df_record_state,
                self._next_load,
                self._next_pv,
                self._next_grid_status,
                self._next_grid_price_import,
            ...
            self._df_record_state = self._update_status(
                control_dict, self._df_record_state, self._next_load, self._next_pv
            )

        if (
            self._tracking_timestep == self._data_length - self.horizon
            or self._tracking_timestep == self._data_length - 1
        ):
            self.done = True
    ...
    def train_test_split(self, train_size=0.67, shuffle=False, cancel=False):
        ...
        if self._has_train_test_split == False:
            ...
            if self.architecture["grid"] == 1:
                ...
                self._grid_price_import_train = self._grid_price_import.iloc[
                    : self._limit_index
                ]
                self._grid_price_import_test = self._grid_price_import.iloc[
                    self._limit_index :
                ]

                self._grid_price_export_train = self._grid_price_export.iloc[
                    : self._limit_index
                ]
                self._grid_price_export_test = self._grid_price_export.iloc[
                    self._limit_index :
                ]
    ...
    def update_variables(self):
        ...
        if self.architecture["grid"] == 1:
            if self._data_set_to_use == "training":
                self.grid.status = self._grid_status_train.iloc[
                    self._tracking_timestep, 0
                ]
                self.grid.price_import = self._grid_price_import_train.iloc[
                    self._tracking_timestep, 0
                ]
                self.grid.price_export = self._grid_price_export_train.iloc[
                    self._tracking_timestep, 0
                ]
                self.grid.co2 = self._grid_co2_train.iloc[self._tracking_timestep, 0]

                self._next_grid_status = self._grid_status_train.iloc[
            ...
            if self._data_set_to_use == "testing":
                ...
                self.grid.price_export = self._grid_price_export_test.iloc[
                    self._tracking_timestep, 0
                ]
                self.grid.co2 = self._grid_co2_test.iloc[self._tracking_timestep, 0]

                self._next_grid_status = self._grid_status_test.iloc[
                    self._tracking_timestep + 1, 0
                ]
            ...
            if self._data_set_to_use == "all":
                ...
                self.grid.price_import = self._grid_price_import.iloc[
                    self._tracking_timestep, 0
                ]
                self.grid.price_export = self._grid_price_export.iloc[
                    self._tracking_timestep, 0
                ]
                self.grid.co2 = self._grid_co2.iloc[self._tracking_timestep, 0]

                if self._tracking_timestep < self._data_length - 1:
                    self._next_grid_status = self._grid_status_ts.iloc[
                        self._tracking_timestep + 1, 0
                    ]
        ...
        if self.architecture["battery"] == 1:
            ...
            self.battery.capa_to_discharge = self._df_record_state["capa_to_discharge"][
                -1
            ]
    ...
    def reset(self, testing=False):
        ...
        self._df_record_state = {
            i: [self._df_record_state[i][0]] for i in self._df_record_state
        }
        ...
        if testing == True and self._data_set_to_use_default == "training":
            ...
            if self._data_set_to_use == "training":
                self._data_length = min(
                    self._load_train.shape[0], self._pv_train.shape[0]
                )
            else:
                self._data_length = min(self._load_ts.shape[0], self._pv_ts.shape[0])
    ...
    def _update_status(
        ...
        if self.architecture["battery"] == 1:
            ...
            capa_to_charge = max(
                (
                    self.parameters["battery_soc_max"].values[0]
                    * self.parameters["battery_capacity"].values[0]
    ...
    def _check_constraints_genset(self, p_genset):
        ...
        if (
            p_genset
            < self.parameters["genset_rated_power"].values[0]
            * self.parameters["genset_pmin"].values[0]
            and p_genset > 1
        ):
            p_genset = (
                self.parameters["genset_rated_power"].values[0]
        ...
        if (
            p_genset
            > self.parameters["genset_rated_power"].values[0]
            * self.parameters["genset_pmax"].values[0]
        ):
            p_genset = (
                self.parameters["genset_rated_power"].values[0]
                * self.parameters["genset_pmax"].values[0]
    ...
    def _check_constraints_battery(self, p_charge, p_discharge, status):
        ...
        if (
            p_charge > capa_to_charge
            or p_charge > self.parameters["battery_power_charge"].values[0]
        ):
            p_charge = min(
                capa_to_charge, self.parameters["battery_power_charge"].values[0]
            )

        if (
            p_discharge > capa_to_discharge
            or p_discharge > self.parameters["battery_power_discharge"].values[0]
        ):
            p_discharge = min(
                capa_to_discharge, self.parameters["battery_power_discharge"].values[0]
            )
    ...
    def _record_production(self, control_dict, df, status):
        ...
        ----------
        control_dict : dictionary
            dictionary representing the control actions taken by an algorithm (either benchmark or in the run function).
        df: dataframe
        ...
        if self.architecture["genset"] == 1:
            try:
                ...
                p_genset = 0
                print(
                    "this microgrid has a genset, you should add a 'genset' field to your control dictionary"
                )
        ...
        if self.architecture["grid"] == 1:
            try:
                ...
                p_export = 0
                print(
                    "this microgrid is grid connected, you should add a 'grid_import' and a 'grid_export' field to your control dictionary"
                )
        ...
        if self.architecture["battery"] == 1:
            try:
                ...
                p_discharge = 0
                print(
                    "this microgrid is grid connected, you should add a 'battery_charge' and a 'battery_discharge' field to your control dictionary"
                )
    ...
    def print_benchmark_cost(self):
        ...
        if self._has_train_test_split:
            self.benchmarks.describe_benchmarks(
                test_split=self._has_train_test_split, test_index=self._limit_index
            )
    ...
    def print_control_info(self):
        ...
        print("you should fill this dictionary at each time step")
    ...
    def penalty(self, coef=1):
        ...
        for i in self._df_record_control_dict:
            penalty += abs(
                self._df_record_control_dict[i][-1]
                - self._df_record_actual_production[i][-1]
            )
